...
async def test_single_low_risk_auto_accept(tmp_project, sample_change, memory_db):
    ...
    with patch("specq.pipeline.LLMCompiler") as MockCompiler:
        mock_compiler_inst = AsyncMock()
        mock_compiler_inst.compile = AsyncMock(return_value="brief")
        MockCompiler.return_value = mock_compiler_inst

        with patch("specq.pipeline.ClaudeCodeExecutor") as MockExec:
            mock_exec_inst = AsyncMock()
            mock_exec_inst.execute = AsyncMock(return_value=_mock_exec_result())
            MockExec.return_value = mock_exec_inst
